...
AnelasticityModel = TypeVar(
    "AnelasticityModel", bound="BaseAnelasticityModel"
)  # define a type hint for any subclass of `BaseAnelasticityModel`
...
class BaseAnelasticityModel(ABC):
    ...
    def build_ghelichkhan_solidus():
        ...
        andrault_solidus = RadialEarthModelFromFile(
            model_name="1d_solidus_Andrault_et_al_2011_EPSL", description="Andrault et al. 2011, EPSL"
        )
...
class CammaranoAnelasticityModel(BaseAnelasticityModel):
    ...
    def from_q_profile(cls, q_profile: str):
        ...
        def B(x):
            return numpy.where(x < 660e3, parameters[q_profile]["B"][0], parameters[q_profile]["B"][1])

        def g(x):
            return numpy.where(x < 660e3, parameters[q_profile]["g"][0], parameters[q_profile]["g"][1])

        def a(x):
            return 0.2 * numpy.ones_like(x)
    ...
    def compute_Q_shear(self, depths: npt.ArrayLike, temperatures: npt.ArrayLike) -> npt.NDArray:
        ...
        Q_values = (
            self.B(depths)
            * (self.omega(depths) ** self.a(depths))
            * numpy.exp((self.a(depths) * self.g(depths) * self.solidus.at_depth(depths)) / temperatures)
        )
...
class GoesAnelasticityModel(BaseAnelasticityModel):
    ...
    def from_q_profile(cls, q_profile: str):
        ...
        """
        parameters = {"Q4": {"Q0": [3.5, 35], "xi": [20, 10]}, "Q6": {"Q0": [0.5, 3.5], "xi": [30, 20]}}

        if q_profile not in parameters.keys():
            raise ValueError(f"Invalid argument: {q_profile}. Must be one of {parameters.keys()}")

        def Q0(x):
            return numpy.where(x < 660e3, parameters[q_profile]["Q0"][0], parameters[q_profile]["Q0"][1])

        def xi(x):
            return numpy.where(x < 660e3, parameters[q_profile]["xi"][0], parameters[q_profile]["xi"][1])

        def a(x):
            return 0.15 * numpy.ones_like(x)
    ...
    def compute_Q_shear(self, depths: npt.ArrayLike, temperatures: npt.ArrayLike) -> npt.NDArray:
        ...
        Q_values = (
            self.Q0(depths)
            * (self.omega(depths) ** self.a(depths))
            * numpy.exp((self.a(depths) * self.xi(depths) * self.solidus.at_depth(depths)) / temperatures)
        )
...
def apply_anelastic_correction(thermo_model: Type[ThermodynamicModel], anelastic_model: Type[AnelasticityModel]):
    ...
    class ThermodynamicModelPrime(thermo_model.__class__):
        ...
        def compute_swave_speed(self):
            ...
            Q_matrix = anelastic_model.compute_Q_shear(depths_x, temperatures_x)
        ...
        def compute_pwave_speed(self):
            ...
            Q_matrix_inv = L / anelastic_model.compute_Q_shear(depths_x, temperatures_x) + (
                1 - L
            ) / anelastic_model.compute_Q_bulk(depths_x, temperatures_x)
    ...
    return ThermodynamicModelPrime(
        thermo_model.model, thermo_model.composition, thermo_model.get_temperatures(), thermo_model.get_depths()
    )
